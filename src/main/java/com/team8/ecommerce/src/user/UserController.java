...
public class UserController {
    ...
    // username parameter가 있다면 해당 회원 조회, 없다면 전체 회원 조회
    @ResponseBody
    ...
    // path variable에 해당하는 회원 조회
    @ResponseBody // Path-variable 방식으로 가져오기
    @GetMapping("/{userIdx}")
    public BaseResponse<GetUserRes> getUser(@PathVariable("userIdx") int userIdx) {
        // @PathVariable RESTful(URL)에서 명시된 파라미터({})를 받는 어노테이션, 이 경우 userIdx값을 받아옴.
